...
app.get("/health", async (req, res) => {
  try {
    const account = contract.getServerAccount();
    res.json({
      ok: true,
      contract: process.env.PROMETHEUS_CONTRACT,
      serverWallet: account.accountAddress.toString(),
      network: "shelbynet",
...
app.post("/api/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    res.json({
      ...
      explorerUrl: `https://explorer.aptoslabs.com/txn/${txHash}?network=shelbynet`,
    });
...
app.listen(PORT, () => {
  ...
  console.log(C.dim(`  Network  : shelbynet\n`));
});
