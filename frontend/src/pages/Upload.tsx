...
import { uploadDoc, formatBytes } from "../lib/api";
...
export default function UploadPage() {
  ...
  const submit = async () => {
    ...
    try {
      const r = await uploadDoc(file, title, desc, stake, setProgress);
      setResult({ docId: r.docId, txHash: r.txHash, explorerUrl: r.explorerUrl });
      setStatus("done");
    } catch (e: any) {
    ...
    <div style={{ minHeight: "100vh", paddingTop: "56px", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <p style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 12, color: "hsl(var(--muted-foreground))", marginBottom: 32 }}>
          Document #{result.docId} is now live on shelbynet
        </p>
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
              <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "hsl(var(--muted-foreground))" }}>
                {progress < 100 ? "Uploading to Shelby..." : "Publishing on Aptos..."}
              </span>
